import { cardId, cardLabel } from "./cards";
...
export type TrickFlowPhase = "playing" | "resolvingTrick" | "clearingTrick" | "betweenTricks" | "handComplete";
...
export const TRICK_RESOLVE_DELAY_MS = 1050;
export const TRICK_CLEAR_DELAY_MS = 140;

export interface TrickPresentationState {
  key: string | null;
  clearedKey: string | null;
  phase: TrickFlowPhase;
}
...
export function createInitialTrickPresentationState(): TrickPresentationState {
  return {
    key: null,
    clearedKey: null,
    phase: "playing"
...
export function buildLatestCompletedTrickKey(state: Pick<GameState, "handNumber" | "completedTricks">): string | null {
  const latestCompleted = state.completedTricks[state.completedTricks.length - 1];
  if (!latestCompleted || latestCompleted.plays.length !== 4 || latestCompleted.winner === undefined) {
    return null;
  }

  return `${state.handNumber}:${state.completedTricks.length}:${latestCompleted.winner}:${latestCompleted.plays
    .map((play) => `${play.player}-${cardId(play.card)}`)
    .join("|")}`;
}

export function shouldPresentLatestCompletedTrick(latestKey: string | null, clearedKey: string | null): boolean {
  return Boolean(latestKey && latestKey !== clearedKey);
}
