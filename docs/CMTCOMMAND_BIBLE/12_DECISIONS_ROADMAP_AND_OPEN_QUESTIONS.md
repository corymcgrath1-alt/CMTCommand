...
| Integration contract v1 uses organization/office scope and individual physical specimen identity. | `contracts/cmtcommand-integration/v1/`, `docs/integration/`, `apps/operational/src/server/integration/cmt-events.ts` | Establishes one locally verified event contract for dispatch, offline field capture, sample/test-set identity, specimen custody, destructive break results, derived averages, review, approval, amendments, and notifications. | Production intake persistence, field app outbox, live lab sync, binary attachment storage, and deployment remain future work. | Yes |
