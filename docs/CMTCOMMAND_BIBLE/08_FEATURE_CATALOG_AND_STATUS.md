...
| Pilot V1 Target | Private assignment media storage foundation | Implemented for local/test foundation | Allow authorized assignment actors to upload private original media and retrieve short-lived private reads. | `src/server/object-storage/`; `src/server/media/`; media tables; `/api/media/*` | Unit, PostgreSQL/service integration, and Playwright signed-upload/read scenarios | Production storage provider, malware scanning, Field Sessions, reports, OCR/AI extraction, samples, retention operations, and legal hold remain unresolved. |
| Pilot V1 Target / Future Integration | CMT integration contract v1 | Implemented for contract/local verification foundation | Define one shared event contract for dispatch, offline field capture, specimen custody, lab results, review, approval, amendments, and operational notifications. | `contracts/cmtcommand-integration/v1/`; `docs/integration/`; `apps/operational/src/server/integration/cmt-events.ts` | Contract simulator test plus Operational vNext contract-drift unit test | Not production persistence, live field/lab sync, binary attachment storage, or deployment. Uses `organizationId`/`officeId` and one `specimenId` per physical cylinder. |
...
| Scope | Feature | Status | User-visible purpose | Main implementation | Tests | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| Future Field Operations | Concrete inspection capture and reporting | Incomplete | Connect an authorized assignment to field evidence, human-reviewed report, sample handoff, and PDF/JSON export. | Architecture/docs only | Runtime tests not implemented | Founder decisions require internal alpha before customer pilot, concrete placement first, assigned-technician attestation, designated technical reviewer approval, customer reporting system as official destination, and Phase 5I production foundation before FR-1. |
| Future Field Operations | Immutable evidence and derivatives | Partially Implemented | Preserve original media and provide usable private previews. | Phase 5G media foundation | Unit, PostgreSQL/service integration, and Playwright signed-upload/read scenarios | Only assignment media primitives exist; future alpha/pilot planning is image-only JPEG/PNG/WebP up to 25 MB, with malware scanning mandatory before customer uploads. Field Sessions, reports, production storage, extraction, retention policy, and legal hold remain incomplete. |
...
- [OPEN QUESTION - High Impact] What monthly infrastructure budget caps should constrain Phase 5I provider selection for internal alpha, one-office customer pilot, and five-office pilot?
